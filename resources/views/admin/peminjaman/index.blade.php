                            ...
                            <td class="py-3 px-4 border-b">
                                <div class="flex flex-col space-y-2">
                                    ...
                                    <form action="{{ route('admin.peminjaman.updateStatus', $item->id) }}" method="POST" class="flex items-center space-x-1">
                                        @csrf
                                        @method('PUT')
                                        <select name="status" onchange="this.form.submit()" class="text-xs border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 transition cursor-pointer">
                                    ...
                                    <form action="{{ route('admin.peminjaman.destroy', $item->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit" 
                                            class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-semibold shadow-sm transition-all duration-200 hover:scale-105 active:scale-95 hover:shadow-md hover:shadow-red-500/20 w-full">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
